...
Pelota::Pelota()
{
    ...
    _tex2.loadFromFile("imagenes/gol.png");
    _gol.setTexture(_tex2);
    _gol.setOrigin(_gol.getGlobalBounds().width / 2, _gol.getGlobalBounds().height / 2);
    _gol.setPosition(600, 400);
...
bool Pelota::verificarGol()
{
    ...
    if (posicionX - _sprite.getGlobalBounds().width / 2 > BORDE_ARCO_IZQUIERDO &&
        ...
        posicionY + _sprite.getGlobalBounds().height / 2 < BORDE_ARCO_INFERIOR && freno)
    {
        resultadoPenal = true;
        return true;
    }
...
void Pelota::reiniciarDisparo()
{
    ...
    if (!tiroEnProceso && freno && posicionY != 730)
    {
        contadorReinicio++;
...
void Pelota::curvaDisparoInferior(float potencia, float movimientoX)
{
    ...
    {
        // Funcion cuadratica
        posicionY = (-0.015f + (potencia * 0.004)) * ((posicionX - POS_INICIAL_PELOTA_X) * (posicionX - POS_INICIAL_PELOTA_X)) +
                    POS_INICIAL_PELOTA_Y;
...
void Pelota::rebote()
{
    const float reboteStep = 4.0f;
    const float posicionObjetivo = 385.0f;

    if (resultadoPenal && freno)
    {
        if (posicionY <= posicionObjetivo)
        {
            posicionY += reboteStep;

            if (posicionY > posicionObjetivo)
            {
                posicionY = posicionObjetivo;
                freno = true;
            }
        }
        else if (posicionY >= posicionObjetivo)
        {
            posicionY -= reboteStep;
